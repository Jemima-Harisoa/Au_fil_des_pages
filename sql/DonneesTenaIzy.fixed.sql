-- Fixed data script: column names aligned to canonical schema
-- Changes made:
-- 1) `employe_id` -> `id_employe`, `competence_id` -> `id_competence`, `date_obtention` -> `date_mesure`
-- 2) Rounded decimal `niveau` values to nearest integer for `employe_competences` inserts
-- 3) `historique_mobilite` column `id_employe` renamed to `id_candidat` to match schema
-- Note: this file preserves the original insert ordering. Foreign-key violations may still occur if referenced parent rows are missing; reordering can be applied if needed.
...
(24, 'Proposer des offres personalizationes et un bon suivi', TRUE),
...
INSERT INTO source_evaluation (libelle, description) VALUES
    ...
    ('certification', 'Validation par un organisme'),
...
('Martin', 'Julien', '1988-09-23', 'julien.m@gmail.com', 1),
('Traore', 'Aïcha', '1995-06-15', 'aicha.traore@outlook.com', 2),
('Rivoire', 'Lucas', '1990-11-30', 'lucas.rivoire@free.fr', 1),
('Rakoto', 'Nirina', '1993-02-18', 'nirina.rakoto@yahoo.com', 2),
('Bernard', 'Sophie', '1985-07-22', 'sophie.bernard@entreprise.com', 2);
...
-- Edited block: align to canonical column names and rounded `niveau`
INSERT INTO employe_competences (id_employe, id_competence, niveau, date_mesure) VALUES
(1, 1, 5, '2023-06-01'), -- Marie → PHP (4.5 rounded to 5)
(1, 2, 5, '2024-01-10'), -- Vue.js (4.8 -> 5)
(1, 3, 4, '2022-09-01'), -- (4.2 -> 4)
(2, 4, 5, '2023-01-01'), -- Julien → Recrutement (4.7 -> 5)
(3, 6, 5, '2021-05-01'); -- Comptable → Comptabilité (4.9 -> 5)
...
-- Edited block: map id_employe -> id_candidat to match canonical historique_mobilite
INSERT INTO historique_mobilite 
    (id_candidat, id_evenement, id_profil, id_departement, date_evenement, support) 
VALUES
